...
pub const Color = struct {
    ...
    pub fn named(name: []const u8) ?Self {
        const v = NAMED_COLORS.get(name) orelse return null;
    ...
    // or just put aliases to its own enum? not sure yet but this is ok for now
    const NAMED_COLORS = blk: {
        @setEvalBranchQuota(5000);
        break :blk std.ComptimeStringMap(u32, .{
            .{ "transparent", 0x0 },
            ...
            .{ "yellowgreen", 0x9acd3ff },
            .{ "rebeccapurple", 0x66339ff },
        });
    };
};
